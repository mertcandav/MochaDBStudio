...
namespace MochaDBStudio.Editor {
    ...
    public class Range:IEnumerable<Place> {
        ...
        /// Constructor
        /// </summary>
        public Range(CodeEditor tb,int iStartChar,int iStartLine,int iEndChar,int iEndLine)
            : this(tb) {
        ...
        /// Constructor
        /// </summary>
        public Range(CodeEditor tb,Place start,Place end)
            : this(tb) {
            this.start = start;
            this.end = end;
        }
        ...
        /// </summary>
        public Range(CodeEditor tb,int iLine)
            : this(tb) {
        ...
        /// FusionTextBox.Selection range. So, if you want to set text, you need to use FusionTextBox.Selection
        /// and FusionTextBox.InsertText() mehtod.
        /// </remarks>
        ...
        /// <param name="regexPattern">Regex pattern</param>
        /// <returns>Enumeration of ranges</returns>
        public Range[] GetArray(string regexPattern) {
            //get text
            string text;
            List<Place> charIndexToPlace;
            GetText(out text,out charIndexToPlace);
            ...
            Range[] Return = new Range[Matches.Count];
            for(int i = 0; i < Matches.Count; i++) {
                Range r = new Range(this.tb);
                //try get 'range' group, otherwise use group 0
                Group group = Matches[i].Groups["range"];
                if(!group.Success)
                    group = Matches[i].Groups[0];
                //
                r.Start = charIndexToPlace[group.Index];
                r.End = charIndexToPlace[group.Index + group.Length];
                Return[i] = r;
        ...
        public IEnumerable<Range> GetRangesByLines(Regex regex) {
            Normalize();
            //
            var fts = tb.TextSource as TextSource; //<----!!!! ugly
            //enumaerate lines
