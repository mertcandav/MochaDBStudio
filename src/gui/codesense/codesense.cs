...
namespace MochaDBStudio.gui.codesense {
  public class codesense:Component, IExtenderProvider, IDisposable {
    ...
    internal codesenseHost Host { get; set; }
    ...
    public ITextBoxWrapper TargetControlWrapper {
      ...
      set {
        ...
        if(value != null && !WrapperByControls.ContainsKey(value.TargetControl)) {
          ...
          SetCodeSense(value.TargetControl,this);
        }
      }
    }
    ...
    public int LeftPadding {
      get {
        if(Host.ListView is codesenseListView)
          return (Host.ListView as codesenseListView).LeftPadding;
        else
          return 0;
      }
      set {
        if(Host.ListView is codesenseListView)
          (Host.ListView as codesenseListView).LeftPadding = value;
      }
    }
    ...
    /// <summary>
    /// MezuaiOtoMenu will popup automatically (when user writes text). Otherwise it will popup only programmatically or by Ctrl-Space.
    /// </summary>
    [DefaultValue(true)]
    public bool AutoPopup { get; set; }

    /// <summary>
    /// MezuaiOtoMenu will capture focus when opening.
    /// </summary>
    [DefaultValue(false)]
    ...
    bool IExtenderProvider.CanExtend(object extendee) {
      //find  MezuaiOtoMenu with lowest hashcode
      if(Container != null)
        foreach(object comp in Container.Components)
          if(comp is codesense)
    ...
    internal virtual void OnSelecting() {
      ...
      Item item = VisibleItems[SelectedItemIndex];
    ...
    private void ApplyAutocomplete(Item item,Range fragment) {
