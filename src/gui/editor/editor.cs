...
namespace MochaDBStudio.gui.editor {
    ...
    /// Code Editor.
    /// </summary>
    public partial class editor:UserControl, ISupportInitialize {
        #region Fields
        ...
        private editor sourceTextBox;
        private Color ShadowColor;
        private int updating;
        private Range updatingRange;
        private Style strongstyle = new MarkerStyle(new SolidBrush(Color.FromArgb(80,Color.DodgerBlue)));
        ...
        public editor() {
            ...
            IndentBackColor = Color.Transparent;
            ...
            WideCaret = false;
            Paddings = new Padding(0,0,200,600);
        ...
        /// <summary>
        /// ShadowTextAreaBorder için renk seçimi.
        /// </summary>
        [DefaultValue(typeof(Color),"Blue")]
        [Description("ShadowTextAreaBorder için renk seçimi.")]
        ...
        [DefaultValue(typeof(HighlightingRangeType),"AllTextRange")]
        ...
        /// Allows to get text from other FusionTextBox.
        /// </summary>
        [Browsable(true)]
        [DefaultValue(null)]
        [Description("Allows to get text from other FusionTextBox.")]
        //[DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
        public editor SourceTextBox {
        ...
        private void SetFont(Font newFont) {
            ...
            //clac size
            SizeF size = GetCharSize(newFont,'M');
        ...
        public void ClearStyle(StyleIndex styleIndex) {
            ...
            for(int i = 0; i < lines.Count; i++) lines[i].ClearStyle(styleIndex);
        ...
        public StyleIndex GetStyleIndexMask(Style[] styles) {
            ...
            for(int i = 0; i < styles.Length; i++) {
                int r = GetStyleIndex(styles[i]);
                if(r >= 0)
                    mask |= Range.ToStyleIndex(r);
            }
        ...
        public void DoAutoIndentChars(int iLine) {
            ...
            for(int i = 0; i < patterns.Length; i++) {
                var m = Regex.Match(this[iLine].Text,patterns[i]);
                if(m.Success) {
                    DoAutoIndentChars(iLine,new Regex(patterns[i]));
                    break;
                }
            }
        }
        ...
        protected override void OnPaint(PaintEventArgs e) {
            ...
            //
            using(Pen servicePen = new Pen(ServiceLinesColor))
            using(Brush changedLineBrush = new SolidBrush(ChangedLineColor))
            using(Brush indentBrush = new SolidBrush(IndentBackColor))
            using(Brush paddingBrush = new SolidBrush(PaddingBackColor))
            using(Brush currentLineBrush =
            new SolidBrush(Color.FromArgb(CurrentLineColor.A == 255 ? 50 : CurrentLineColor.A,CurrentLineColor))) {
                ...
                for(iLine = startLine; iLine < lines.Count; iLine++) {
                    ...
                    if(CurrentLineColor != Color.Transparent & iLine == Selection.Start.iLine)
                        if(Selection.IsEmpty)
                            e.Graphics.FillRectangle(currentLineBrush,
                                                     new Rectangle(textAreaRect.Left,y,
                                                     (textAreaRect.Right+Paddings.Right)-textAreaRect.Left
                                                     ,CharHeight));
                    //CurrentLineBorder
                    if(iLine == Selection.Start.iLine)
                        if(Selection.IsEmpty)
                            e.Graphics.DrawRectangle(new Pen(Color.Gray,2),
                                new Rectangle(textAreaRect.Left,y,
                                (textAreaRect.Right+Paddings.Right)-textAreaRect.Left,CharHeight));
                    ...
                    if(ShowLineNumbers)
                        ...
                        using(StringFormat stringFormat = new StringFormat(StringFormatFlags.DirectionRightToLeft) { LineAlignment = StringAlignment.Center })
                        using(Font font = new Font(Font.FontFamily,Font.Size))
        ...
        private void DrawTextAreaBorder(Graphics graphics) {
            ...
            if(TextAreaBorder == TextAreaBorderType.Shadow) {
                ...
                using(var brush = new SolidBrush(Color.FromArgb(80,ShadowColor))) {
        ...
        protected void HighlightEquals() {
            Range.ClearStyle(strongstyle);
            if(!Selection.IsEmpty) return;
            string text = Selection.GetFragment(@"\w").Text;
            if(text.Length == 0) return;
            Range[] ranges = Range.GetArray("\\b" + text + "\\b");
            if(ranges.Length > 1)
                /*foreach (var r in ranges)
                    r.SetStyle(VurguStyle);*/
                for(int i = 0; i < ranges.Length; i++) ranges[i].SetStyle(strongstyle);
        }
            ...
            switch(FindEndOfFoldingBlockStrategy) {
                case FindEndOfFoldingBlockStrategy.Strategy1:
                    for(i = iStartLine + 1; i < LinesCount; i++) {
                        ...
                        if(marker == lines[i].FoldingEndMarker)
